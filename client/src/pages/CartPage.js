...
const CartPage = () => {
    ...
    const updateCartItemQuantity = (productId, newQuantity) => {
        try {
            ...
            if (index !== -1) {
                // Ensure the quantity is valid, between 1 and the available quantity
                myCart[index].quantity = Math.max(1, newQuantity);
                setCart(myCart);
            ...
            <div className="container">
                <div className="row mt-2">
                    <div className="col-md-12">
                        <div className="text-center p-2" style={{ backgroundColor: "lightblue", borderRadius: '5px' }}>
                            <p style={{ fontWeight: 'bold', color: "red", borderRadius: '5px', fontSize: "28px", display: 'inline' }}>
                                {`Welcome ${auth?.token && auth?.user?.name}`}
                            </p>
                            {cart?.length ? (
                                <span style={{ color: "black", fontSize: "20px", marginLeft: '10px', padding: "4px" }}>
                ...
                <div className="row mt-2">
                                ...
                                <div className="col-md-8 mt-3 mt-md-0">
                                    ...
                                    <div className="d-flex align-items-center mb-2">
                                        ...
                                        <button
                                            ...
                                            disabled={p.quantity <= 1} // Disable if quantity is 1
                                        >
                                            -
                                        </button>
                                        <span className="me-2">{p.quantity || 1}</span>
                                        <button
                                            className="btn btn-sm btn-outline-secondary"
                                            onClick={() => updateCartItemQuantity(p._id, p.quantity + 1)}
                                            disabled={p.quantity >= p.availableQuantity} // Disable if quantity is at max available quantity
                                        >
                                            +
                                        </button>
                                    </div>
                    ...
                    <div className="col-md-4 text-center  mb-2 mt-3" style={{ backgroundColor: "#F1F1F1", borderRadius: '8px', border: '1px dashed gray' }}>
                        <h5 className='text-primary mt-2'>Cart Summary </h5>
                        <p>Total | Checkout | Payment</p>
                        <hr />
                        <h6>Total: <span className='text-danger'>{totalPrice()}</span></h6>
                        {auth?.user?.address ? (
                            <div className="mb-3">
